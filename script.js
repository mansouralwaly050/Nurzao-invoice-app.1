...
async function renderToPDF(html, filename){
  if(typeof html2canvas === 'undefined') throw new Error('مكتبة html2canvas لم يتم تحميلها (تحقق من الإنترنت)');
  if(typeof window.jspdf === 'undefined') throw new Error('مكتبة jsPDF لم يتم تحميلها (تحقق من الإنترنت)');
...
async function exportInvoicePDF(invoiceId){
  ...
  try{
    ...
    toast('⚠️ فشل إنشاء PDF: ' + err.message);
  }
}
